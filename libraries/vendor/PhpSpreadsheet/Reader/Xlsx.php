...
use TablePress\PhpOffice\PhpSpreadsheet\Reader\Xlsx\AutoFilter;
use TablePress\PhpOffice\PhpSpreadsheet\Reader\Xlsx\Chart;
use TablePress\PhpOffice\PhpSpreadsheet\Reader\Xlsx\ColumnAndRowAttributes;
use TablePress\PhpOffice\PhpSpreadsheet\Reader\Xlsx\ConditionalStyles;
use TablePress\PhpOffice\PhpSpreadsheet\Reader\Xlsx\DataValidations;
...
use TablePress\PhpOffice\PhpSpreadsheet\Worksheet\HeaderFooterDrawing;
...
class Xlsx extends BaseReader
{
					...
					if ($xmlWorkbookNS->sheets) {
						foreach ($xmlWorkbookNS->sheets->sheet as $eleSheet) {
							...
							if (!$this->readDataOnly) {
								...
								if ($unparsedVmlDrawings) {
									foreach ($unparsedVmlDrawings as $rId => $relPath) {
										/** @var mixed[][][] $unparsedLoadedData */
										$rId = (string) substr($rId, 3); // rIdXXX
										/** @var mixed[][] */
							...
							if (str_starts_with($drawingFilename, 'xl//xl/')) {
								$drawingFilename = (string) substr($drawingFilename, 4);
							}
							if (str_starts_with($drawingFilename, '/xl//xl/')) {
								$drawingFilename = (string) substr($drawingFilename, 5);
							}
								...
								foreach ($relsWorksheet->Relationship as $elex) {
									...
									if ((string) $ele['Type'] === "$xmlNamespaceBase/drawing") {
										...
										if (str_starts_with($eleTarget, '/xl/')) {
											$drawings[(string) $ele['Id']] = (string) substr($eleTarget, 1);
										} else {
										...
										if ($relsDrawing && $relsDrawing->Relationship) {
											foreach ($relsDrawing->Relationship as $elex) {
												...
												if ($eleType === "$xmlNamespaceBase/image") {
													$eleTarget = (string) $ele['Target'];
													if (str_starts_with($eleTarget, '/xl/')) {
														$eleTarget = (string) substr($eleTarget, 1);
													...
													if ($this->includeCharts) {
														$eleTarget = (string) $ele['Target'];
														if (str_starts_with($eleTarget, '/xl/')) {
															$index = (string) substr($eleTarget, 1);
														} else {
	...
	/** @param null|bool|mixed[]|SimpleXMLElement $array
				 * @param int|string $key */
				private static function getArrayItemString($array, $key = 0): string
	{
	...
	/** @param null|bool|mixed[]|SimpleXMLElement $array
				...
				private static function getArrayItemIntOrSxml($array, $key = 0)
	{
		$retVal = self::getArrayItem($array, $key);
	...
	private function readHyperLinkDrawing(\TablePress\PhpOffice\PhpSpreadsheet\Worksheet\Drawing $objDrawing, SimpleXMLElement $cellAnchor, array $hyperlinks): void
	{
