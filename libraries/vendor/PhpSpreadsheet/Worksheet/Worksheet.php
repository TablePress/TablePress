...
use TablePress\PhpOffice\PhpSpreadsheet\Cell\Hyperlink;
...
use TablePress\PhpOffice\PhpSpreadsheet\Collection\CellsFactory;
...
use TablePress\PhpOffice\PhpSpreadsheet\Shared;
...
class Worksheet
{
	...
	{
		$activeSheet = ($nullsafeVariable2 = $this->getParent()) ? $nullsafeVariable2->getActiveSheetIndex() : null;
		...
		if ($activeSheet !== null && $activeSheet >= 0) {
			($nullsafeVariable3 = $this->getParent()) ? $nullsafeVariable3->setActiveSheetIndex($activeSheet) : null;
		}
	 ...
	 * @return $this
	 */
	public function setSheetState(string $value)
	{
	 ...
	 * @return $this
	 */
	public function removeConditionalStyles(string $coordinate)
	{
	 ...
	 * @return $this
	 */
	public function setConditionalStyles(string $coordinate, array $styles)
	{
	 ...
	 * @return $this
	 */
	public function duplicateStyle(Style $style, string $range)
	{
	 ...
	 * @return $this
	 */
	public function setBreak($coordinate, int $break, int $max = -1)
	{
		$cellAddress = Functions::trimSheetFromCellReference(Validations::validateCellAddress($coordinate));
	 ...
	 * @return $this
	 */
	public function mergeCells($range, string $behaviour = self::MERGE_CELL_CONTENT_EMPTY)
	{
	 ...
	 * @return $this
	 */
	public function protectCells($range, string $password = '', bool $alreadyHashed = false, string $name = '', string $securityDescriptor = '')
	{
	 ...
	 *               or as an array of [$columnIndex, $row] (e.g. [3, 5]), or a CellAddress object.
	 */
	public function getComment($cellCoordinate, bool $attachNew = true): Comment
	{
		$cellAddress = Functions::trimSheetFromCellReference(Validations::validateCellAddress($cellCoordinate));
	 ...
	 * @return $this
	 */
	public function setSelectedCell(string $coordinate)
	{
	 ...
	 * @return $this
	 */
	public function setRightToLeft(bool $value)
	{
	 ...
	 * @return $this
	 */
	public function fromArray(array $source, $nullValue = null, string $startCell = 'A1', bool $strictNullComparison = false)
	{
				...
				protected function cellToArray(Cell $cell, bool $calculateFormulas, bool $formatData, $nullValue, bool $lessFloatPrecision = false)
	{
		...
		if ($cell->getValue() !== null) {
			...
			if ($formatData) {
				...
				$returnValue = NumberFormat::toFormattedString($returnValuex, $style->getNumberFormat()->getFormatCode() ?? NumberFormat::FORMAT_GENERAL, null, $lessFloatPrecision);
			}
		}
	...
	/**
		...
		if ($namedRange !== null) {
			$cellRange = ltrim((string) substr($namedRange->getValue(), (int) strrpos($namedRange->getValue(), '!')), '!');
	 ...
	 */
	public static function extractSheetTitle(?string $range, bool $returnRange = false, bool $unapostrophize = false)
	{
		if (empty($range)) {
		...
		if ($returnRange) {
			$title = (string) substr($range, 0, $sep);
			if ($unapostrophize) {
				$title = self::unApostrophizeTitle($title);
			}

			return [$title, (string) substr($range, $sep + 1)];
		}

		return (string) substr($range, $sep + 1);
	}
	...
	public static function unApostrophizeTitle(?string $title): string
	{
		...
		if ($title[0] === "'" && substr($title, -1) === "'") {
			$title = str_replace("''", "'", (string) substr($title, 1, -1));
		}
	 ...
	 * @return $this
	 */
	public function setDataValidation(string $cellCoordinate, ?DataValidation $dataValidation = null)
	{
	 ...
	 * @return $this
	 */
	public function setCodeName(string $codeName, bool $validate = true)
	{
		// Is this a 'rename' or not?
