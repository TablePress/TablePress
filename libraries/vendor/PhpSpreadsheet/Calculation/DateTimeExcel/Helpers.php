...
class Helpers
{
	...
	/**
				 ...
				 */
				public static function getDateValue($dateValue, bool $allowBool = true, ?int $calendar = null): float
	{
		if (is_object($dateValue)) {
			$retval = SharedDateHelper::PHPToExcel($dateValue, $calendar);
			if (is_bool($retval)) {
		...
		try {
			SharedDateHelper::excelToDateTimeObject((float) $dateValue, null, $calendar);
	...
	{
		// Execute function
		$PHPDateObject = SharedDateHelper::excelToDateTimeObject($dateValue, null, $calendar);
	...
	/**
				 ...
				 */
				public static function returnIn3FormatsArray(array $dateArray, bool $noFrac = false, ?int $calendar = null)
	{
		...
		return SharedDateHelper::excelToTimestamp($excelDateValue, null, $calendar);
	}
	...
	/**
				 ...
				 */
				public static function returnIn3FormatsFloat(float $excelDateValue, ?int $calendar = null)
	{
		$retType = Functions::getReturnDateType();
		if ($retType === Functions::RETURNDATE_EXCEL) {
		...
		return SharedDateHelper::excelToDateTimeObject($excelDateValue, null, $calendar);
	}
	...
	/**
				 ...
				 * @return \DateTime|float|int
				 */
				public static function returnIn3FormatsObject(DateTime $PHPDateObject, ?int $calendar = null)
	{
		$retType = Functions::getReturnDateType();
		if ($retType === Functions::RETURNDATE_PHP_DATETIME_OBJECT) {
			return $PHPDateObject;
		...
		if ($retType === Functions::RETURNDATE_EXCEL) {
			return (float) SharedDateHelper::PHPToExcel($PHPDateObject, $calendar);
		}
		// RETURNDATE_UNIX_TIMESTAMP
		$stamp = SharedDateHelper::PHPToExcel($PHPDateObject, $calendar);
		$stamp = is_bool($stamp) ? ((int) $stamp) : $stamp;

		return SharedDateHelper::excelToTimestamp($stamp, null, $calendar);
	}
	...
	/**
				 ...
				 * @param mixed $number
				 */
				public static function nullFalseTrueToNumber(&$number, bool $allowBool = true, ?int $calendar = null): void
	{
