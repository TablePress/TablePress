...
use TablePress\Composer\Pcre\Preg; // many pregs in this program use u modifier, which has side-effects which make it unsuitable for this
use TablePress\PhpOffice\PhpSpreadsheet\Calculation\Engine\BranchPruner;
use TablePress\PhpOffice\PhpSpreadsheet\Calculation\Engine\CyclicReferenceStack;
use TablePress\PhpOffice\PhpSpreadsheet\Calculation\Engine\Logger;
use TablePress\PhpOffice\PhpSpreadsheet\Calculation\Engine\Operands;
...
class Calculation extends CalculationLocale
{
	...
	public static function getExcelConstants(string $key): ?bool
	{
	...
	/**
				 * @param mixed $enabled
				 */
				public function setBranchPruningEnabled($enabled): self
	{
	...
	/**
				 ...
				 * @return mixed
				 */
				public function calculateCellValue(?Cell $cell = null, bool $resetLog = true)
	{
		if ($cell === null) {
			return null;
		}
	...
	/**
				 ...
				 * @return mixed
				 */
				public function calculateFormula(string $formula, ?string $cellID = null, ?Cell $cell = null)
	{
	...
	/**
				 * @param mixed $cellValue
				 */
				public function saveValueToCache(string $cellReference, $cellValue): void
	{
	...
	/**
				 ...
				 * @return mixed
				 */
				public function _calculateFormulaValue(string $formula, ?string $cellID = null, ?Cell $cell = null, bool $ignoreQuotePrefix = false)
	{
		$cellValue = null;
	 ...
	 */
	private function internalParseFormula(string $formula, ?Cell $cell = null)
	{
		...
		while (true) {
			...
			$isOperandOrFunction = (bool) preg_match($regexpMatchString, (string) substr($formula, $index), $match);
			...
			if ($opCharacter === '-' && !$expectingOperator) {                //    Is it a negation instead of a minus?
				...
				if (preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/miu', $val, $matches)) {
					...
					$ax = preg_match('/^\s*\)/u', (string) substr($formula, $index + $length));
					if ($ax) {
					...
					if ($testPrevOp !== null && $testPrevOp['value'] === ':') {
						...
						preg_match('/^' . self::CALCULATION_REGEXP_ROW_RANGE . '/miu', (string) substr($formula, $index), $rowRangeReference)
					) {
						...
						preg_match('/^' . self::CALCULATION_REGEXP_COLUMN_RANGE . '/miu', (string) substr($formula, $index), $columnRangeReference)
					) {
			...
			if ($formula[$index] === ' ') {
				...
				if (
					...
					&& (
						(preg_match('/^' . self::CALCULATION_REGEXP_CELLREF . '.*/miu', (string) substr($formula, $index), $match))
							&& ($output[$countOutputMinus1]['type'] === 'Cell Reference')
						|| (preg_match('/^' . self::CALCULATION_REGEXP_DEFINEDNAME . '.*/miu', (string) substr($formula, $index), $match))
							&& ($output[$countOutputMinus1]['type'] === 'Defined Name' || $output[$countOutputMinus1]['type'] === 'Value')
						|| (preg_match('/^' . self::CALCULATION_REGEXP_STRUCTURED_REFERENCE . '.*/miu', (string) substr($formula, $index), $match))
	...
	private function processTokenStack($tokens, ?string $cellID = null, ?Cell $cell = null)
	{
		...
		$originalCoordinate = ($nullsafeVariable1 = $cell) ? $nullsafeVariable1->getCoordinate() : null;
	...
	/**
				 ...
				 * @return mixed
				 */
				private function executeNumericBinaryOperation($operand1, $operand2, string $operation, Stack &$stack)
	{
	...
	/**
				 * @return mixed
				 */
				private function evaluateDefinedName(Cell $cell, DefinedName $namedRange, Worksheet $cellWorksheet, Stack $stack, bool $ignoreScope = false)
	{
		...
		$y = ($nullsafeVariable2 = $namedRange->getWorksheet()) ? $nullsafeVariable2->getTitle() : null;
	...
	/**
				 ...
				 * @return mixed
				 */
				public static function boolToString($operand1)
	{
		if (is_bool($operand1)) {
