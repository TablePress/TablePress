...
namespace TablePress\PhpOffice\PhpSpreadsheet\Calculation\Web;
...
class Service
{
	/**
				 ...
				 * @param mixed $url
				 */
				public static function webService($url, ?Cell $cell = null): ?string
	{
		if (is_array($url)) {
		...
		$domainWhiteList = (($nullsafeVariable1 = ($nullsafeVariable2 = $cell) ? $nullsafeVariable2->getWorksheet()->getParent() : null) ? $nullsafeVariable1->getDomainWhiteList() : null) ?? [];
		$host = $parsed['host'] ?? '';
