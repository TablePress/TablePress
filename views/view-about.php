...
class TablePress_About_View extends TablePress_View {
	...
	public function postbox_usage( $data, $box ) {
		...
		<p><?php printf( __( 'To insert the table into a page, post or text-widget, copy the shortcode %s and paste it into the corresponding place in the editor.', 'tablepress' ), '<input type="text" class="table-shortcode table-shortcode-inline" value="[table id=&lt;ID&gt; /]" readonly="readonly" />' ); ?> <?php printf( __( 'You can also select the desired table from a list (after clicking the button &quot;%s&quot; in the editor toolbar) and the corresponding Shortcode will be added for you.', 'tablepress' ), __( 'Table', 'tablepress' ) ); ?></p>
