...
b.shiftKey).closest("tr");0===b.length?alert(tablepress_strings.no_rows_selected):(b.each(function(b,d){var e=a(d),f=e.find("textarea"),h=e.clone();h.find("textarea").removeAttr("id").each(function(b,c){a(c).val(f.eq(b).val())});e.after(h)}),tp.rows.stripe(),tp.reindex())},hide:function(b){b=a("#edit-form-body").find("input:checked").prop("checked",b.shiftKey).closest("tr");0===b.length?alert(tablepress_strings.no_rows_selected):(b.addClass("row-hidden").find(".visibility").val("0"),tp.rows.stripe(),
...
1:-1;typeof e!==typeof f&&(e+="",f+="");if(e<f)return-1;if(e>f)return 1}return 0};a.each(h,function(c,d){d.sort_key=(""+a(d).children().eq(b).find("textarea").val()).toLowerCase()});h.sort(function(a,b){return c*g(a.sort_key,b.sort_key)});a.each(h,function(a,b){b.sort_key=null});d.append(e);d.append(h);d.append(f);tp.rows.stripe();tp.reindex()},stripe:function(b){"undefined"==typeof b&&(b=null);b=a(b);var c=a("#edit-form-body").children().removeClass("odd head-row foot-row").not(b);c.filter(":even").addClass("odd");
c=c.not(".row-hidden");b.hasClass("row-hidden")&&(c=c.not(".ui-sortable-placeholder"));tp.table.head&&c.first().addClass("head-row");tp.table.foot&&c.last().addClass("foot-row")}},columns:{append:function(){var b,c=a("#columns-append-number").val(),d="",e="",f="";if(/^[1-9][0-9]{0,4}$/.test(c)){for(b=0;b<c;b++)e+=tp.table.body_cell,d+=tp.table.head_cell,f+=tp.table.foot_cell;a("#edit-form-body").children().each(function(b,c){a(c).children().slice(-tp.table.no_data_columns_post).before(e)});a("#edit-form-head").children().slice(-tp.table.no_data_columns_post).before(d);
a("#edit-form-foot").children().slice(-tp.table.no_data_columns_post).before(f);tp.reindex()}else alert(tablepress_strings.append_num_columns_invalid),a("#columns-append-number").focus().select()},insert:function(b){var c;b=a("#edit-form-foot").find("input:checked").prop("checked",b.shiftKey).closest("th");0===b.length?alert(tablepress_strings.no_columns_selected):(c=b.map(function(){return a(this).index()}).get(),a("#edit-form-body").children().each(function(b,e){a(e).children().filter(function(b){return-1!=
a.inArray(b,c)}).before(tp.table.body_cell)}),a("#edit-form-head").children().filter(function(b){return-1!=a.inArray(b,c)}).before(tp.table.head_cell),b.before(tp.table.foot_cell),tp.reindex())},duplicate:function(b){var c;b=a("#edit-form-foot").find("input:checked").prop("checked",b.shiftKey).closest("th");0===b.length?alert(tablepress_strings.no_columns_selected):(c=b.map(function(){return a(this).index()}).get(),a("#edit-form").find("tr").each(function(b,e){a(e).children().each(function(b,d){if(-1!=
a.inArray(b,c)){var e=a(d),l=e.clone();l.find("textarea").removeAttr("id").val(e.find("textarea").val());e.after(l)}})}),tp.reindex())},hide:function(b){var c;b=a("#edit-form-foot").find("input:checked").prop("checked",b.shiftKey).closest("th");0===b.length?alert(tablepress_strings.no_columns_selected):(c=b.map(function(){return a(this).index()}).get(),a("#edit-form-body").children().add("#edit-form-head").each(function(b,e){a(e).children().filter(function(b){return-1!=a.inArray(b,c)}).addClass("column-hidden")}),
b.addClass("column-hidden").find(".visibility").val("0"),tp.table.set_table_changed())},unhide:function(b){var c;b=a("#edit-form-foot").find("input:checked").prop("checked",b.shiftKey).closest("th");0===b.length?alert(tablepress_strings.no_columns_selected):(c=b.map(function(){return a(this).index()}).get(),a("#edit-form-body").children().add("#edit-form-head").each(function(b,e){a(e).children().filter(function(b){return-1!=a.inArray(b,c)}).removeClass("column-hidden")}),b.removeClass("column-hidden").find(".visibility").val("1"),
tp.table.set_table_changed())},remove:function(){var b,c,d=a("#edit-form-foot").find("input:checked").closest("th");0===d.length?alert(tablepress_strings.no_columns_selected):tp.table.columns==d.length?alert(tablepress_strings.no_remove_all_columns):(c=1==d.length?tablepress_strings.ays_remove_columns_singular:tablepress_strings.ays_remove_columns_plural,confirm(c)&&(b=d.map(function(){return a(this).index()}).get(),a("#edit-form-body").children().add("#edit-form-head").each(function(c,d){a(d).children().filter(function(c){return-1!=
a.inArray(c,b)}).remove()}),d.remove(),tp.reindex()))},move:{source_idx:-1,target_idx:-1,$rows:null,$row_children:null,$cell:null,$cells:null,$placeholder:null,$helper:null,start:function(b,c){var d=a(c.item);tp.columns.move.source_idx=d.index();tp.columns.move.$rows=a("#edit-form-body").children().add("#edit-form-foot");tp.columns.move.$cells=tp.columns.move.$rows.find(":nth-child("+(tp.columns.move.source_idx+1)+")").each(function(){tp.columns.move.$cell=a(this);a('<td class="move-placeholder"><div/></td>').insertBefore(tp.columns.move.$cell);
...
tp.save_changes.success(a)},ajax_error:function(a,c,d){tp.save_changes.error("AJAX call failed: "+c+" - "+d+". Try again while holding down the &#8220;Shift&#8221; key.")},success:function(b){tp.table.id!=b.table_id&&"pushState"in window.history&&null!==window.history.pushState&&window.history.pushState("","",window.location.href.replace(/table_id=[0-9a-zA-Z-_]+/gi,"table_id="+b.table_id));a("#edit-form").removeClass("tablepress-edit-screen-id-"+tp.table.id).addClass("tablepress-edit-screen-id-"+
b.table_id);tp.table.id=tp.table.new_id=b.table_id;a("#table-id").val(tp.table.id);a("#table-new-id").val(tp.table.new_id);a(".table-shortcode").val("["+tablepress_options.shortcode+" id="+tp.table.new_id+" /]");a("#nonce-edit-table").val(b.new_edit_nonce);a("#nonce-preview-table").val(b.new_preview_nonce);var c=a(".show-preview-button");c.length&&c.attr("href",c.first().attr("href").replace(/item=[a-zA-Z0-9_-]+/g,"item="+b.table_id).replace(/&_wpnonce=[a-z0-9]+/ig,"&_wpnonce="+b.new_preview_nonce));
a("#last-modified").text(b.last_modified);a("#last-editor").text(b.last_editor);tp.table.unset_table_changed();tp.save_changes.after_saving_dialog("success",tablepress_strings[b.message])},error:function(a){tp.save_changes.after_saving_dialog("error",a)},after_saving_dialog:function(b,c){c="undefined"==typeof c?"":": "+c;var d,e="save-changes-"+b;"success"==b?(e+=" updated",d=3E3):(e+=" error",d=6E3);a(".animation-saving").closest("p").after('<div class="'+e+'"><p><strong>'+tablepress_strings["save_changes_"+
b]+c+"</strong></p></div>");a(".animation-saving").remove();a(".save-changes-"+b).delay(d).fadeOut(2E3,function(){a(this).remove()});a(".save-changes-button").prop("disabled",!1);a("body").removeClass("wait")}},init:function(){var b={click:{"#rows-insert":tp.rows.insert,"#columns-insert":tp.columns.insert,"#rows-duplicate":tp.rows.duplicate,"#columns-duplicate":tp.columns.duplicate,"#rows-remove":tp.rows.remove,"#columns-remove":tp.columns.remove,"#rows-hide":tp.rows.hide,"#columns-hide":tp.columns.hide,
"#rows-unhide":tp.rows.unhide,"#columns-unhide":tp.columns.unhide,"#rows-append":tp.rows.append,"#columns-append":tp.columns.append,"#link-add":tp.content.link.add,"#image-add":tp.content.image.add,"#span-add-rowspan":function(){tp.content.span.add("#rowspan#")},"#span-add-colspan":function(){tp.content.span.add("#colspan#")},".show-preview-button":tp.table.preview.trigger,".save-changes-button":tp.save_changes.trigger,".show-help-box":function(){a(this).next().wpdialog({title:a(this).attr("title"),
height:400,width:300,modal:!0,dialogClass:"wp-dialog",resizable:!1})}},keyup:{"#table-new-id":tp.check.table_id},change:{"#option-table-head":tp.table.change_table_head,"#option-table-foot":tp.table.change_table_foot,"#option-use-datatables":tp.table.change_datatables,"#option-datatables-paginate":tp.table.change_datatables_pagination},blur:{"#table-new-id":tp.table.change_id}},c=a("#edit-form-body");a.each(b,function(b,c){a.each(c,function(c,e){a(c).on(b,e)})});a(window).on("beforeunload",tp.check.changes_saved);
a("#option-table-head").change();a("#option-print-name").change(tp.table.change_print_name_description).change();a("#option-print-description").change(tp.table.change_print_name_description).change();c.one("change","textarea",tp.table.set_table_changed);a("#tablepress_edit-table-information, #tablepress_edit-table-options, #tablepress_edit-datatables-features").one("change","input, textarea, select",tp.table.set_table_changed);tablepress_options.cells_advanced_editor?(c.on("click","textarea",tp.cells.advanced_editor.keyopen),
a("#advanced-editor-open").on("click",tp.cells.advanced_editor.buttonopen),a("#advanced-editor-confirm").on("click",tp.cells.advanced_editor.save),a("#advanced-editor-cancel").on("click",tp.cells.advanced_editor.close),a("#advanced-editor").wpdialog({autoOpen:!1,title:a("#advanced-editor-open").val(),width:600,modal:!0,dialogClass:"wp-dialog",resizable:!1})):a("#advanced-editor-open").hide();if(tablepress_options.cells_auto_grow)c.on("focus","textarea",tp.cells.autogrow);a("#edit-form-body").on("click",
"input:checkbox",{parent:"#edit-form-body"},tp.cells.checkboxes.multi_select);a("#edit-form-foot").on("click","input:checkbox",{parent:"#edit-form-foot"},tp.cells.checkboxes.multi_select);a("#edit-form-head").on("click",".sort-control",tp.rows.sort);a("#tablepress-page").find("form").on("submit",function(){a(this).find(".tablepress-postbox-table").find("input, select").prop("disabled",!1)});c.sortable({axis:"y",containment:a("#edit-form"),forceHelperSize:!0,handle:".move-handle",start:tp.rows.move.start,
