...
sort:function(){var b=a(this).parent().index(),c=a(this).hasClass("sort-asc")?1:-1,d=a("#edit-form-body"),e=d.find(".head-row").prevAll().addBack(),f=d.find(".foot-row").nextAll().addBack(),h=d.children().not(e).not(f).get(),g=function(a,b){var c=/(^-?[0-9]+(\.?[0-9]*)[df]?e?[0-9]?$|^0x[0-9a-f]+$|[0-9]+)/gi,d=/(^[ ]*|[ ]*$)/g,e=/(^([\w ]+,?[\w ]+)?[\w ]+,?[\w ]+\d+:\d+(:\d+)?[\w ]?|^\d{1,4}[\/\-]\d{1,4}[\/\-]\d{1,4}|^\w+, \w+ \d+, \d{4})/,f=/^0x[0-9a-f]+$/i,h=/^0/,g=a.replace(d,"")||"",j=b.replace(d,
"")||"",d=g.replace(c,"\x00$1\x00").replace(/\0$/,"").replace(/^\0/,"").split("\x00"),c=j.replace(c,"\x00$1\x00").replace(/\0$/,"").replace(/^\0/,"").split("\x00"),g=parseInt(g.match(f))||1!=d.length&&g.match(e)&&Date.parse(g);if(e=parseInt(j.match(f))||g&&j.match(e)&&Date.parse(j)||null){if(g<e)return-1;if(g>e)return 1}g=0;for(j=Math.max(d.length,c.length);g<j;g++){e=!(d[g]||"").match(h)&&parseFloat(d[g])||d[g]||0;f=!(c[g]||"").match(h)&&parseFloat(c[g])||c[g]||0;if(isNaN(e)!==isNaN(f))return isNaN(e)?
