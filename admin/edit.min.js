...
tp.columns.move.$cells=tp.columns.move.$placeholder=tp.columns.move.$helper=null;tp.reindex()}},number_to_letter:function(a){for(var b="";0<a;)b=String.fromCharCode(65+(a-1)%26)+b,a=Math.floor((a-1)/26);return b}},cells:{$focus:c(null),$textarea:null,autogrow:function(){tp.cells.$focus.removeClass("focus");tp.cells.$focus=c(this).closest("tr").addClass("focus")},advanced_editor:{keyopen:function(a){a.shiftKey&&(a=$id("advanced-editor-content"),tp.cells.advanced_editor.thickbox_size(),tp.cells.$textarea=
c(this).blur(),a.val(tp.cells.$textarea.val()),$id("advanced-editor").wpdialog("open"),a.get(0).selectionStart=a.get(0).selectionEnd=a.val().length,a.focus())},buttonopen:function(){if(confirm(tablepress_strings.advanced_editor_open))$id("edit-form-body").one("click","textarea",function(){var a=$id("advanced-editor-content");tp.cells.advanced_editor.thickbox_size();tp.cells.$textarea=c(this).blur();a.val(tp.cells.$textarea.val());$id("advanced-editor").wpdialog("open");a.get(0).selectionStart=a.get(0).selectionEnd=
a.val().length;a.focus()})},save:function(){var a=$id("advanced-editor-content").blur().val();tp.cells.$textarea.val()!=a&&(tp.cells.$textarea.val(a),tp.cells.$textarea.get(0).selectionStart=tp.cells.$textarea.get(0).selectionEnd=tp.cells.$textarea.val().length,tp.table.set_table_changed());tp.cells.$textarea.focus();tp.cells.advanced_editor.close()},close:function(){$id("advanced-editor").wpdialog("close");return!1},thickbox_size:function(){var a=$id("advanced-editor-content-add_media"),b;b=c(window).width();
var d=720<b?720:b,e=c(window).height();a.length&&(b=a.attr("href"),c("body.admin-bar").length&&(e-=28),b=b.replace(/&width=[0-9]+/g,"").replace(/&height=[0-9]+/g,""),a.attr("href",b+("&width="+(d-80)+"&height="+(e-85))))}},checkboxes:{last_clicked:{"#edit-form-body":!1,"#edit-form-foot":!1},multi_select:function(a){if("undefined"==a.shiftKey)return!0;if(a.shiftKey){if(!tp.cells.checkboxes.last_clicked[a.data.parent])return!0;var b=c(a.data.parent).find(":checkbox"),d=b.index(tp.cells.checkboxes.last_clicked[a.data.parent]),
e=b.index(this);d!=e&&b.slice(Math.min(d,e),Math.max(d,e)).prop("checked",c(this).prop("checked"))}tp.cells.checkboxes.last_clicked[a.data.parent]=this;return!0}}},content:{link:{add:function(){if(confirm(tablepress_strings.link_add))$id("edit-form-body").one("mousedown","textarea",function(){wpActiveEditor=this.id;c(document).one("mouseup",function(){wpLink.open();tp.table.set_table_changed()})})}},image:{add:function(){if(confirm(tablepress_strings.image_add))$id("edit-form-body").one("click","textarea",
function(){wpActiveEditor=this.id;this.selectionStart=this.selectionEnd=this.value.length;var a=$id("image-add"),b=c(window).width(),b=720<b?720:b,d=c(window).height();c("body.admin-bar").length&&(d-=28);tb_show(a.text(),a.attr("href")+"&TB_iframe=true&height="+(d-85)+"&width="+(b-80),!1);c(this).blur()});return!1}},span:{add:function(a){var b="#rowspan#"==a?tablepress_strings.rowspan_add:tablepress_strings.colspan_add;if($id("option-use-datatables").prop("checked"))if(confirm(tablepress_strings.span_add_datatables_warning))$id("option-use-datatables").prop("checked",
!1).change();else return;if(confirm(b))$id("edit-form-body").one("click","textarea",function(){var b=c(this),e=b.parent().index(),f=b.closest("tr").index();if("#rowspan#"==a){if(0===f){alert(tablepress_strings.no_rowspan_first_row);return}if(tp.table.head&&1==f){alert(tablepress_strings.no_rowspan_table_head);return}if(tp.table.foot&&tp.table.rows-1==f){alert(tablepress_strings.no_rowspan_table_foot);return}}else if("#colspan#"==a&&tp.table.no_data_columns_pre==e){alert(tablepress_strings.no_colspan_first_col);
return}b.val(a);tp.table.set_table_changed()})}}},check:{table_id:function(a){37!=a.which&&39!=a.which&&(a=c(this),a.val(a.val().replace(/[^0-9a-zA-Z-_]/g,"")))},changes_saved:function(){if(tp.made_changes)return tablepress_strings.unsaved_changes_unload}},reindex:function(){var a,b=$id("edit-form-body").children(),d,e={};tp.table.rows=b.length;tp.table.columns=0<tp.table.rows?b.first().children().length-tp.table.no_data_columns_pre-tp.table.no_data_columns_post:0;b.each(function(b,h){a=c(h);a.find("textarea").val(function(a,
b){return""===b||"="!=b.charAt(0)?b:b.replace(/([A-Z]+[0-9]+)(?::([A-Z]+[0-9]+))?/g,function(a,b,c){e.hasOwnProperty(b)||(d=$id("cell-"+b),e[b]=d.length?tp.columns.number_to_letter(d.parent().index()-tp.table.no_data_columns_pre+1)+(d.closest("tr").index()+1):b);a=e[b];"undefined"!=typeof c&&""!==c&&(e.hasOwnProperty(c)||(d=$id("cell-"+c),e[c]=d.length?tp.columns.number_to_letter(d.parent().index()-tp.table.no_data_columns_pre+1)+(d.closest("tr").index()+1):c),a+=":"+e[c]);return a})}).attr("name",
function(a){return"table[data]["+b+"]["+a+"]"});a.find(".move-handle").html(b+1)}).each(function(a,b){c(b).find("textarea").attr("id",function(b){return"cell-"+tp.columns.number_to_letter(b+1)+(a+1)})});$id("edit-form-head").find(".move-handle").html(function(a){return tp.columns.number_to_letter(a+1)});$id("number-rows").val(tp.table.rows);$id("number-columns").val(tp.table.columns);tp.table.set_table_changed()},save_changes:{trigger:function(a){$id("option-datatables-paginate").prop("checked")&&
