...
tp.columns.number_to_letter(a+1)+(b+1)})});a("#edit-form-head").find(".move-handle").html(function(a){return tp.columns.number_to_letter(a+1)});a("#number-rows").val(tp.table.rows);a("#number-columns").val(tp.table.columns);tp.table.set_table_changed()},save_changes:{trigger:function(b){a("#option-datatables-paginate").prop("checked")&&!/^[1-9][0-9]{0,4}$/.test(a("#option-datatables-paginate_entries").val())?(alert(tablepress_strings.num_pagination_entries_invalid),a("#option-datatables-paginate_entries").focus().select()):
/[^A-Za-z0-9- _]/.test(a("#option-extra-css-classes").val())?(alert(tablepress_strings.extra_css_classes_invalid),a("#option-extra-css-classes").focus().select()):b.shiftKey?(tp.made_changes=!1,a("#tablepress-page").find("form").submit()):(a(this).closest("p").append('<span class="animation-saving spinner" title="'+tablepress_strings.saving_changes+'"/>'),a(".save-changes-button").prop("disabled",!0),a("body").addClass("wait"),a.post(ajaxurl,tp.table.prepare_ajax_request("tablepress_save_table","#nonce-edit-table"),
function(){},"json").success(tp.save_changes.ajax_success).error(tp.save_changes.ajax_error))},ajax_success:function(a,c){"undefined"==typeof c||"success"!=c?tp.save_changes.error("AJAX call successful, but unclear status. Try again while holding down the &#8220;Shift&#8221; key."):"undefined"==typeof a||null==a||"-1"==a||"undefined"==typeof a.success||!0!==a.success?tp.save_changes.error("AJAX call successful, but unclear data. Try again while holding down the &#8220;Shift&#8221; key."):tp.save_changes.success(a)},
ajax_error:function(a,c,d){tp.save_changes.error("AJAX call failed: "+c+" - "+d+". Try again while holding down the &#8220;Shift&#8221; key.")},success:function(b){tp.table.id!=b.table_id&&"pushState"in window.history&&null!==window.history.pushState&&window.history.pushState("","",window.location.href.replace(/table_id=[0-9a-zA-Z-_]+/gi,"table_id="+b.table_id));tp.table.id=tp.table.new_id=b.table_id;a("#table-id").val(tp.table.id);a("#table-new-id").val(tp.table.new_id);a(".table-shortcode").val("["+
tablepress_options.shortcode+" id="+tp.table.new_id+" /]");a("#nonce-edit-table").val(b.new_edit_nonce);a("#nonce-preview-table").val(b.new_preview_nonce);var c=a(".show-preview-button");c.length&&c.attr("href",c.first().attr("href").replace(/item=[a-zA-Z0-9_-]+/g,"item="+b.table_id).replace(/&_wpnonce=[a-z0-9]+/ig,"&_wpnonce="+b.new_preview_nonce));a("#last-modified").text(b.last_modified);a("#last-editor").text(b.last_editor);tp.table.unset_table_changed();tp.save_changes.after_saving_dialog("success",
