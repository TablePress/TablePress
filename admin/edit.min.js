...
c.find("head").append(b.head_html);c.find("body").append(b.body_html)}).appendTo("#table-preview");a(".animation-preview").remove();a("body").removeClass("wait");tp.table.preview.show("#TB_inline?inlineId=preview-container")},error:function(b){a(".animation-preview").closest("p").after('<div class="preview-error error"><p><strong>'+tablepress_strings.preview_error+": "+b+"</strong></p></div>");a(".animation-preview").remove();a(".preview-error").delay(6E3).fadeOut(2E3,function(){a(this).remove()});
...
"",window.location.href.replace(/table_id=[0-9a-zA-Z-_]+/gi,"table_id="+b.table_id));tp.table.id=tp.table.new_id=b.table_id;a("#table-id").val(tp.table.id);a("#table-new-id").val(tp.table.new_id);a(".table-shortcode").val("["+tablepress_options.shortcode+" id="+tp.table.new_id+" /]");a("#nonce-edit-table").val(b.new_edit_nonce);a("#nonce-preview-table").val(b.new_preview_nonce);var c=a(".show-preview-button");c.length&&c.attr("href",c.first().attr("href").replace(/item=[a-zA-Z0-9_-]+/g,"item="+b.table_id).replace(/&_wpnonce=[a-z0-9]+/ig,
"&_wpnonce="+b.new_preview_nonce));a("#last-modified").text(b.last_modified);a("#last-editor").text(b.last_editor);tp.table.unset_table_changed();tp.save_changes.after_saving_dialog("success",tablepress_strings[b.message])},error:function(a){tp.save_changes.after_saving_dialog("error",a)},after_saving_dialog:function(b,c){var c="undefined"==typeof c?"":": "+c,d,e="save-changes-"+b;"success"==b?(e+=" updated",d=3E3):(e+=" error",d=6E3);a(".animation-saving").closest("p").after('<div class="'+e+'"><p><strong>'+
